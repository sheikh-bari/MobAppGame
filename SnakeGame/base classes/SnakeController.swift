...
final class SnakeController: SCNNode {
    // MARK: - Properties
    ...
    func turnLeft() {
        if(direction == .up || direction == .down) {
            direction = SnakeDirection(rawValue: 3)!
        }
    }
    
    func turnRight() {
        if(direction == .up || direction == .down) {
            direction = SnakeDirection(rawValue: 1)!
        }
    }
    
    func turnUp() {
        if(direction == .right || direction == .left) {
            direction = SnakeDirection(rawValue: 2)!
        }
    }
    
    func turnDown() {
        if(direction == .right || direction == .left) {
            direction = SnakeDirection(rawValue: 0)!
        }
    }
...
final class Warm: SCNNode {
    
    var warmNode: SCNNode?
    
    override init() {
        super.init()
        if let scene = SCNScene(named: "food.scn"), let warmNode = scene.rootNode.childNode(withName: "food", recursively: true) {
            addChildNode(warmNode)
        }
    }
